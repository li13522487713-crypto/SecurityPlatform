...
const playwrightArgs = ["test", "-c", "playwright.e2e-all.config.ts", ...process.argv.slice(2)];
...
function log(message) {
  process.stdout.write(`[run-all-e2e] ${message}\n`);
}
...
async function runPlaywright() {
  ...
  const playwrightSpawnArgs = [playwrightCli, ...playwrightArgs];
  const exitCode = await new Promise((resolve, reject) => {
    const child = spawn(playwrightCommand, playwrightSpawnArgs, {
      cwd: frontendRoot,
      env: {
        ...process.env,
  ...
  if (exitCode !== 0) {
    throw new Error(`全量 E2E 失败，退出码 ${exitCode}`);
  }
}
