  ...
  return (
    <Select
      ...
      optionList={microflows.map(microflow => ({
        label: `${microflow.name || microflow.qualifiedName} · ${microflow.qualifiedName} · ${microflow.moduleName} (${microflow.parameters.length} params -> ${dataTypeKind(microflow.returnType)})`,
        value: microflow.id,
        disabled: microflow.id === currentMicroflowId,
        showTick: true,
        render: () => (
          <Space vertical spacing={0} align="start">
            <Text strong>{microflow.name || microflow.qualifiedName}</Text>
            <Text type="tertiary" size="small">{microflow.qualifiedName} · {microflow.moduleName} · {microflow.status ?? "draft"}</Text>
            {microflow.id === currentMicroflowId ? <Text type="warning" size="small">Cannot call itself directly in Stage 15.</Text> : null}
          </Space>
        ),
      }))}
      onChange={selected => {
