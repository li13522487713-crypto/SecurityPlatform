import type { MicroflowSchema, MicroflowValidationIssue, MicroflowVariableSymbol } from "../schema/types";
...
export function validateVariables(schema: MicroflowSchema): MicroflowValidationIssue[] {
  ...
  for (const group of Object.values(schema.variables ?? {})) {
    for (const symbol of Object.values(group) as MicroflowVariableSymbol[]) {
