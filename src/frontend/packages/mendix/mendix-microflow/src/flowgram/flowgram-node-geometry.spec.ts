...
describe("flowgram node geometry", () => {
  it("keeps declared FlowGram node sizes aligned with compact Mendix DOM geometry", () => {
    ...
    expect(getMendixMicroflowNodeSize("continueEvent")).toEqual({ width: 18, height: 18 });
    expect(getMendixMicroflowNodeSize("breakEvent")).toEqual({ width: 18, height: 18 });
    expect(getMendixMicroflowNodeSize("errorEvent")).toEqual({ width: 22, height: 22 });
    expect(getMendixMicroflowNodeSize("parameterObject")).toEqual({ width: 100, height: 40 });
