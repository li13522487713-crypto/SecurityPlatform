...
const activityDefinitions: Array<Parameters<typeof activityEntry>[0]> = [
  ...
  { activityType: "declareLocalVariable", title: "Local Variable", titleZh: "局部变量", activityCategory: "variable", config: { variableName: "newVar", scope: "local", dataType: { kind: "string" }, source: "empty" }, description: "声明局部或全局作用域的变量。", supportsErrorHandling: false },
  ...
  { activityType: "queryExternalDatabase", title: "Database Query", titleZh: "数据库查询", activityCategory: "integration", config: { databaseSourceId: "", sql: "", output: { variableName: "queryResult", kind: "list" } }, description: "通过资源库数据源执行参数化 SQL，输出对象/列表/数组到变量。" },
...
export const microflowObjectNodeRegistries: MicroflowNodeRegistryEntry[] = [
  ...
  createEntry({
    ...
    ports: [sequenceIn, sequenceOut, port("bodyIn", "Body In", "output", "loopBodyIn", "one", ["loopBody"]), port("bodyOut", "Body Out", "input", "loopBodyOut", "zeroOrMore", ["sequence"]), errorOut],
  ...
  createEntry({
    ...
    titleZh: "并行网关",
    description: "Splits the flow into parallel branches and joins them later; runtime executes all reachable branches and records branch trace.",
    category: "decisions",
    group: "Decisions",
    ...
    documentation: doc("Parallel split / join. The testRun engine executes all reachable branches and records branch trace."),
  ...
  createEntry({
    ...
    titleZh: "包含网关",
    description: "Routes to any branches whose conditions are true and records selected or skipped branch trace.",
    category: "decisions",
    group: "Decisions",
    ...
    documentation: doc("Inclusive (OR) gateway. The testRun engine selects every branch whose condition is true and records branch trace."),
