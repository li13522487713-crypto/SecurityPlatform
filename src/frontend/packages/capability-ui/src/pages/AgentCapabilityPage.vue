<template>
  <section class="capability-page">
    <header class="capability-page__header">
      <h2 class="capability-page__title">Agent</h2>
    ...
    <div class="capability-page__actions">
      <button type="button" class="capability-action" @click="goTo(agentListPath)">AgentList</button>
      <button type="button" class="capability-action" @click="goTo(agentChatPath)">AgentChat</button>
    </div>
  </section>
</template>
...
const agentListPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/ai/agents`;
  }
  return "/ai/agents";
});

const agentChatPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/ai/chat`;
  }
  return "/ai/agents";
});
