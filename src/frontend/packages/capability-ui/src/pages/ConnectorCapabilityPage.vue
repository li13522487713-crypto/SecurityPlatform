<template>
  <section class="capability-page">
    <header class="capability-page__header">
      <h2 class="capability-page__title">Connector</h2>
    ...
    <div class="capability-page__actions">
      <button type="button" class="capability-action" @click="goTo(configPath)">ConnectorConfig</button>
      <button type="button" class="capability-action" @click="goTo(exposurePath)">ConnectorExposure</button>
    </div>
  </section>
</template>
...
const configPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/connectors/config`;
  }
  return "/console/connectors/list";
});

const exposurePath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/connectors/exposure`;
  }
  return "/console/connectors/capabilities";
});
