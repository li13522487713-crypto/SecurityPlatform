<template>
  <section class="capability-page">
    <header class="capability-page__header">
      <h2 class="capability-page__title">Data</h2>
    ...
    <div class="capability-page__actions">
      <button type="button" class="capability-action" @click="goTo(dataPath)">DynamicTables</button>
      <button type="button" class="capability-action" @click="goTo(designerPath)">DataDesigner</button>
    </div>
  </section>
</template>
...
const dataPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/data`;
  }
  if (context.value.appId) {
    return `/apps/${context.value.appId}/data`;
  }
  return "/apps";
});

const designerPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/data/designer`;
  }
  if (context.value.appId) {
    return `/apps/${context.value.appId}/data/designer`;
  }
  return "/apps";
});
