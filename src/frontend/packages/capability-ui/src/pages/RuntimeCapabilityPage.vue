<template>
  <section class="capability-page">
    <header class="capability-page__header">
      <h2 class="capability-page__title">Runtime</h2>
    ...
    <div class="capability-page__actions">
      <button type="button" class="capability-action" @click="goTo(contextPath)">RuntimeContext</button>
      <button type="button" class="capability-action" @click="goTo(executionPath)">RuntimeExecution</button>
    </div>
  </section>
</template>
...
const contextPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/approval`;
  }
...
const executionPath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/visualization`;
  }
