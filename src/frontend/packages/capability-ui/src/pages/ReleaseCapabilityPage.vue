<template>
  <section class="capability-page">
    <header class="capability-page__header">
      <h2 class="capability-page__title">Release</h2>
    ...
    <div class="capability-page__actions">
      <button type="button" class="capability-action" @click="goTo(releasePath)">ReleaseCenter</button>
      <button type="button" class="capability-action" @click="goTo(resourcePath)">ResourceCenter</button>
    </div>
  </section>
</template>
...
const releasePath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/settings`;
  }
...
const resourcePath = computed(() => {
  if (context.value.hostMode === "app" && context.value.appKey) {
    return `/apps/${context.value.appKey}/profile`;
  }
