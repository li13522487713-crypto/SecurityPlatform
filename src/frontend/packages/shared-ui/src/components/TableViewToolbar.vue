...
import type { TableViewController, TableViewDensity } from "@atlas/shared-core";
...
const viewOptions = computed(() =>
  props.controller.state.views.map((item) => ({
    label: item.isDefault
      ? `${item.name} (${t("tableView.defaultSuffix")})`
      : item.name,
    value: item.id,
  }))
);
...
const filteredColumns = computed(() => {
  ...
  return props.controller.columnSettings.filter((item) =>
...
const treeData = computed(() => {
  return filteredColumns.value.map((col) => ({
    ...
    children: (col as unknown as Record<string, unknown>).children,
  }));
});
...
const handleTreeCheck = (checked: unknown) => {
  const checkedKeyList: string[] = Array.isArray(checked)
    ? checked
    : (checked as { checked: string[] }).checked;
...
const handleTreeDrop = (_info: unknown) => {
  // Tree drag-drop reorder placeholder
};

const densityLabel = computed(() => {
  if (props.controller.state.density === "compact")
    return t("tableView.densityCompact");
  if (props.controller.state.density === "comfortable")
    return t("tableView.densityComfortable");
