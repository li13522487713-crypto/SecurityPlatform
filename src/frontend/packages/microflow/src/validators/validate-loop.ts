...
export function validateLoop(schema: MicroflowSchema): MicroflowValidationIssue[] {
  ...
  for (const { object, loopObjectId } of flattenObjects(schema.objectCollection)) {
    ...
    if ((object.kind === "breakEvent" || object.kind === "continueEvent")) {
      const outgoing = schema.flows.filter(flow => flow.originObjectId === object.id);
      if (outgoing.length > 0) {
        issues.push(issue("MF_LOOP_CONTROL_OUTGOING", "Break/Continue events cannot have outgoing flows.", { objectId: object.id }));
      }
    }
  }
  return issues;
