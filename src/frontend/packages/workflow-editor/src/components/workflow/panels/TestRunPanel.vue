  ...
  <div class="test-run-panel">
    ...
    <div v-show="activeTab === 'run'" class="panel-body">
      ...
      <div class="run-output-section">
        <div class="section-title">{{ t('wfUi.testRun.watchVariables') }}</div>
        <div v-if="watchVariables.length === 0" class="log-empty">{{ t('wfUi.testRun.noVariableSnapshot') }}</div>
...
const { t } = useI18n()
import type { WorkflowVersionItem, NodeExecutionItem, ExecutionStatus } from '../../../types'
import type { StreamRunHandle, WorkflowTestRunClient } from '../../../api'
...
const props = defineProps<{
  ...
  apiClient: WorkflowTestRunClient
}>()
...
async function startRun(mode: 'sync' | 'stream') {
  ...
  interruptQuestion.value = ''
    ...
    try {
      ...
      const runRes = await props.apiClient.runSync(props.workflowId, { inputs })
      ...
      const processRes = await props.apiClient.getProcess(result.executionId)
    ...
    try {
      streamHandle = props.apiClient.runStream(
        props.workflowId,
        { inputs },
        {
...
async function submitAnswer() {
  ...
  try {
    await props.apiClient.resume(currentExecutionId.value, { data: { answer: interruptAnswer.value } })
    interruptQuestion.value = ''
...
async function refreshExecutionDetail(executionId: string) {
  const processRes = await props.apiClient.getProcess(executionId)
  if (!processRes.success || !processRes.data) {
    return
  }
