<template>
  <a-form layout="vertical" size="small">
    <a-form-item :label="t('wfUi.forms.start.defaultVariables')">
      <div v-for="(item, index) in variables" :key="index" class="variable-row">
        <a-input v-model:value="item.name" :placeholder="t('wfUi.forms.start.variableName')" @change="emitChange" />
        ...
        <a-input v-model:value="item.defaultValue" :placeholder="t('wfUi.forms.start.defaultValue')" @change="emitChange" />
        <a-button size="small" @click="removeVariable(index)">-</a-button>
      </div>
      <a-button size="small" type="dashed" @click="addVariable">{{ t('wfUi.forms.start.addVariable') }}</a-button>
    </a-form-item>

    <a-form-item :label="t('wfUi.forms.start.autoSaveHistory')">
