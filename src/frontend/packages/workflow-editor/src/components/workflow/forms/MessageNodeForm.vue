<template>
  <a-form layout="vertical" size="small">
    <a-form-item :label="t('wfUi.forms.message.nodeType')">
      <a-tag color="cyan">{{ nodeType }}</a-tag>
    </a-form-item>
    <a-form-item :label="t('wfUi.forms.message.conversationId')">
      <a-input v-model:value="conversationId" placeholder="conversation_id" @change="emitChange" />
    </a-form-item>
    <a-form-item v-if="nodeType === 'EditMessage' || nodeType === 'DeleteMessage'" :label="t('wfUi.forms.message.messageId')">
      <a-input v-model:value="messageId" placeholder="message_id" @change="emitChange" />
    </a-form-item>
    <a-form-item v-if="nodeType === 'CreateMessage' || nodeType === 'EditMessage'" :label="t('wfUi.forms.message.content')">
      <a-textarea v-model:value="content" :rows="4" @change="emitChange" />
    </a-form-item>
    <a-form-item v-if="nodeType === 'MessageList'" :label="t('wfUi.forms.message.pageSize')">
      <a-input-number v-model:value="pageSize" :min="1" :max="100" style="width: 100%" @change="emitChange" />
    </a-form-item>
    <a-form-item :label="t('wfUi.forms.message.outputKey')">
