...
import { getTenantDataSources } from "@/services/api";
import {
...
async function loadDataSourceOptions() {
  try {
    const rows = await getTenantDataSources();
    dataSourceOptions.value = rows
      .filter((item: TenantDataSourceDto) => item.isActive)
      .map((item: TenantDataSourceDto) => ({
