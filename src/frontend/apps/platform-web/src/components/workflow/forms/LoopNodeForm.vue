  ...
  <a-form layout="vertical" size="small">
    <a-form-item label="循环模式">
      <a-select v-model:value="mode" @change="emitChange">
        <a-select-option value="count">按次数</a-select-option>
        <a-select-option value="while">条件循环</a-select-option>
        <a-select-option value="forEach">遍历集合</a-select-option>
      </a-select>
    </a-form-item>
    ...
    <a-form-item v-if="mode === 'while'" label="循环条件表达式">
      <a-textarea v-model:value="condition" :rows="3" placeholder="例如: {{loop_index}} < 10" @change="emitChange" />
    </a-form-item>

    <template v-if="mode === 'forEach'">
      <a-form-item label="集合变量路径">
        <a-input v-model:value="collectionPath" placeholder="例如: input.items" @change="emitChange" />
      </a-form-item>
      <a-form-item label="元素变量名">
        <a-input v-model:value="itemVariable" placeholder="loop_item" @change="emitChange" />
      </a-form-item>
      <a-form-item label="元素索引变量名">
