...
import type { ApprovalHistoryEventDto, ApprovalInstanceDetailDto } from "@/types/approval-instance-detail";
import type { FlowDefinition, FlowPublishResponse, FlowSaveRequest, FlowSaveResponse, FlowValidationResult } from "@/types/workflow";
...
export async function getApprovalFlowsPaged(pagedRequest: PagedRequest) {
  ...
  const response = await requestApi<ApiResponse<PagedResult<ApprovalFlowDefinitionListItem>>>(`/approval/flows?${query}`);
  if (!response.data) {
...
export async function compareApprovalFlowVersion(id: string, targetVersion: number) {
  const response = await requestApi<ApiResponse<ApprovalFlowCompareResponse>>(
    `/approval/flows/${id}/versions/${targetVersion}/compare`
  );
  if (!response.data) {
...
export async function getApprovalFlowVersionDetail(id: string, versionId: string): Promise<ApprovalFlowVersionDetail> {
  const response = await requestApi<ApiResponse<ApprovalFlowVersionDetail>>(
    `/approval/flows/${id}/versions/${versionId}/detail`
  );
  if (!response.data) {
...
export async function delegateTask(taskId: string, delegateeUserId: string, comment?: string) {
  const response = await requestApi<ApiResponse<string>>(
    `/approval/tasks/${taskId}/delegation?delegateeUserId=${delegateeUserId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function communicateTask(taskId: string, recipientUserId: string, content: string) {
  const response = await requestApi<ApiResponse<string>>(
    `/approval/tasks/${taskId}/communication?recipientUserId=${recipientUserId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function batchTransferTasks(fromUserId: string, toUserId: string) {
  const response = await requestApi<ApiResponse<string>>(
    `/approval/tasks/batch-transfer?fromUserId=${fromUserId}&toUserId=${toUserId}`,
    {
      method: "POST"
    }
  );
