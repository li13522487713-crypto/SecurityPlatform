...
export async function reportClientError(request: ClientErrorReportRequest): Promise<void> {
  ...
  if (!response.success) throw new Error(response.message || "上报失败");
}

export async function getUsersPaged(pagedRequest: PagedRequest) {
  const query = toQuery(pagedRequest, {
    DepartmentId:
      pagedRequest.departmentId !== undefined && pagedRequest.departmentId !== null
        ? String(pagedRequest.departmentId)
        : undefined
  });
...
export async function updateUserRoles(id: string, request: UserAssignRolesRequest) {
  ...
  if (!response.success) throw new Error(response.message || "更新角色失败");
}
...
export async function updateUserDepartments(id: string, request: UserAssignDepartmentsRequest) {
  ...
  if (!response.success) throw new Error(response.message || "更新部门失败");
}
...
export async function updateUserPositions(id: string, request: UserAssignPositionsRequest) {
  ...
  if (!response.success) throw new Error(response.message || "更新职位失败");
}

export async function deleteUser(id: string) {
  const response = await requestApi<ApiResponse<{ id: string }>>(`/users/${id}`, { method: "DELETE" });
...
export async function deleteDepartment(id: string) {
  const response = await requestApi<ApiResponse<{ id: string }>>(`/departments/${id}`, { method: "DELETE" });
  if (!response.success) throw new Error(response.message || "删除失败");
}
...
export async function deleteRole(id: string) {
  const response = await requestApi<ApiResponse<{ id: string }>>(`/roles/${id}`, { method: "DELETE" });
  if (!response.success) throw new Error(response.message || "删除失败");
}
...
export async function updateRolePermissions(id: string, request: RoleAssignPermissionsRequest) {
  ...
  if (!response.success) throw new Error(response.message || "更新权限失败");
}
...
export async function updateRoleMenus(id: string, request: RoleAssignMenusRequest) {
  ...
  if (!response.success) throw new Error(response.message || "更新菜单失败");
}
...
export async function setRoleDataScope(id: string, dataScope: number, deptIds?: number[]) {
  ...
  if (!response.success) throw new Error(response.message || "更新数据权限失败");
}

export async function getPermissionsPaged(pagedRequest: PermissionQueryRequest) {
  const query = toQuery(pagedRequest, { type: pagedRequest.type });
...
export async function deletePosition(id: string) {
  const response = await requestApi<ApiResponse<{ id: string }>>(`/positions/${id}`, { method: "DELETE" });
  if (!response.success) throw new Error(response.message || "删除失败");
}
