...
import type { TableViewApiFunctions } from "@atlas/shared-core";
...
export async function getTableViewsPaged(
  ...
  const response = await requestApi<
    ApiResponse<PagedResult<TableViewListItem>>
...
export async function getDefaultTableViewConfig(
  tableKey: string
...
export async function getTableViewDetail(
  id: string
): Promise<TableViewDetail> {
  const response = await requestApi<ApiResponse<TableViewDetail>>(
    `/table-views/${id}`
  );
  if (!response.data) {
    throw new Error(response.message || "查询失败");
...
export async function createTableView(
  request: TableViewCreateRequest
): Promise<TableViewListItem> {
  const response = await requestApi<ApiResponse<TableViewListItem>>(
    "/table-views",
    {
      method: "POST",
...
export async function updateTableView(
  id: string,
  request: TableViewUpdateRequest
): Promise<void> {
