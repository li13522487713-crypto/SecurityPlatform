...
function mapRawToVisible(raw: RawConsoleMenuItem): ConsoleMenuItem | null {
  if (raw.children) {
    ...
    return { key: raw.key, label: t(raw.labelKey), children: filtered };
  }
...
const visibleMenuItems = computed(() =>
  menuItemsRaw.map(mapRawToVisible).filter(Boolean) as ConsoleMenuItem[]
);
...
const flatMenuItems = computed(() => {
  ...
  menuItemsRaw.forEach((item) => {
    if (item.path) {
      result.push({ key: item.key, label: t(item.labelKey), path: item.path, permission: item.permission });
    }
    if (item.children) {
      item.children.forEach((child) => {
        if (child.path) {
          result.push({ key: child.key, label: t(child.labelKey), path: child.path, permission: child.permission });
        }
      });
    }
  });
...
const selectedKeys = computed(() => {
  ...
  if (matched && hasPermission(matched.permission)) return [matched.key];
  return [];
});
...
function onMenuClick(info: { key: string }) {
  if (!info.key.startsWith("group-")) go(info.key);
}
...
.left { display: flex; align-items: center; flex: 1; min-width: 0; overflow: hidden; }
.brand { color: var(--color-text-primary); font-weight: 600; font-size: 16px; margin-right: 32px; cursor: pointer; white-space: nowrap; flex-shrink: 0; }
.menu-wrapper { flex: 1; min-width: 0; }
