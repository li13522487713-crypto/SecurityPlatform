...
import { getRouters } from "@/services/api-auth";
...
function buildRoutesFromRouters(routers: RouterVo[]): RouteRecordRaw[] {
  return routers.map((r) => ({
    path: r.path,
    name: r.name,
    meta: r.meta,
    children: r.children ? buildRoutesFromRouters(r.children) : undefined
  } as RouteRecordRaw));
}

function buildRouteState(routers: RouterVo[]) {
  const normalized = normalizeRouters(routers);
  const builtRoutes = buildRoutesFromRouters(normalized);
  return {
    sidebarRouters: normalized,
    addRoutes: builtRoutes,
    routes: builtRoutes,
    defaultRoutes: builtRoutes,
    topbarRouters: builtRoutes
  };
}
  ...
  actions: {
    ...
    hydrateFromLocalCache() {
      ...
      try {
        ...
        if (cached) routers = JSON.parse(cached) as RouterVo[];
      } catch {
        routers = null;
      }
    ...
    async refreshRoutes(router?: Router) {
      if (generateRoutesInflight) return await generateRoutesInflight;
      generateRoutesInflight = (async () => {
        try {
          ...
          this.routeLoaded = true;
          this.routerLoadFailed = false;
          if (router) this.registerRoutes(router);
    ...
    registerRoutes(router: Router) {
      ...
      for (const route of this.addRoutes) {
        ...
        if (route.path && existingPaths.has(route.path)) continue;
