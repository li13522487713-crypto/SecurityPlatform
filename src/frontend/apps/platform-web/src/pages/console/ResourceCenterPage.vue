...
import { getResourceCenterDataSourceConsumption, getResourceCenterGroups } from "@/services/api-console";
import type {
...
async function loadResourceCenterData() {
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : t("console.resourceCenter.loadFailed"));
  } finally {
    loading.value = false;
  }
}
