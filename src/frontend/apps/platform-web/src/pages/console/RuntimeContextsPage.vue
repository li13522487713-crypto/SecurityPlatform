...
async function loadRuntimeContexts() {
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : t("console.runtimeCtx.loadFailed"));
  } finally {
    loading.value = false;
  }
}
...
async function viewDetail(appKey: string, pageKey: string) {
  try {
    ...
    message.error(error instanceof Error ? error.message : t("console.runtimeCtx.loadDetailFailed"));
  }
}
