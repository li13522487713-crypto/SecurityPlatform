  ...
  try {
    ...
    workspaceCards.value = cards.map((card) => {
      ...
      return {
        ...
        memberNames: detail.members.map(
          (member) => agentNameMap.get(String(member.agentId)) || member.alias || `#${member.agentId}`
        )
      };
    });
...
function goChat(card: WorkspaceAgentCard, entrySkill?: string) {
  ...
  if (entrySkill) {
    void router.push({
      name: "ai-agent-edit",
      params: { id: String(card.sourceId) },
      query: { entrySkill }
    });
    return;
  }
  void router.push({
    name: "ai-agent-chat",
    params: { agentId: String(card.sourceId) }
  });
}
