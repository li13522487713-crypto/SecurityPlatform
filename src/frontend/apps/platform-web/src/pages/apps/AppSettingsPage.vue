      ...
      <a-col :span="24">
        <a-card :title="t('appsSettings.cardDataSource')">
          ...
          <a-space class="mt12">
            ...
            <a-button v-if="!dataSourceInfo?.dataSourceId" :loading="bindingDataSource" @click="handleOpenBindDataSource">
              {{ t("appsSettings.bindDs") }}
            </a-button>
            <a-button v-else :loading="bindingDataSource" @click="handleOpenSwitchDataSource">
...
import { debounce, type TenantDataSourceDto } from "@atlas/shared-core";
...
async function loadSettings() {
  ...
  try {
    ...
    entityAliases.value = aliases.length > 0 ? aliases : defaultEntityAliases();
  } catch (error) {
...
async function ensureTenantAppDetail() {
  ...
  const latestDetail = await getTenantAppInstanceDetail(appId.value);
  if (!isMounted.value) {
    return null;
  }
  tenantAppDetail.value = latestDetail;
  return latestDetail;
}
...
async function updateAppDataSourceBinding(targetDataSourceId: number | null) {
  ...
  if (!isMounted.value || !currentDetail) {
    return;
  }
...
async function handleTestDataSource() {
  ...
  try {
    ...
    const latestDataSource = await getTenantAppInstanceDataSourceInfo(appId.value);
    if (!isMounted.value) {
      return;
    }
    dataSourceInfo.value = latestDataSource;
  } catch (error) {
...
async function saveFileStorageSettings() {
  ...
  try {
    ...
    const latestSettings = await getTenantAppInstanceFileStorageSettings(appId.value);
    if (!isMounted.value) {
      return;
    }
    Object.assign(fileStorageForm, latestSettings);
  } catch (error) {
