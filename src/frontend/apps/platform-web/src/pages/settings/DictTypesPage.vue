<template>
  <div style="padding: 24px;">
    <a-row :gutter="16">
      <a-col :span="8">
        <a-card :title="t('dict.titleTypes')">
          <template #extra>
          ...
          <a-spin :spinning="typeLoading">
            <a-list :data-source="typeList" :locale="{ emptyText: t('dict.emptyTypes') }">
              <template #renderItem="{ item }">
                <a-list-item
                  style="cursor: pointer; padding: 8px 12px; border-radius: 6px;"
                  :style="{ background: selectedType?.id === item.id ? '#e6f4ff' : undefined }"
                ...
                >
                  ...
                  <template #actions>
                    ...
                    <a-popconfirm :title="t('dict.deleteTypeConfirm')" @confirm="handleDeleteType(item.id)">
      ...
      <a-col :span="16">
        ...
        >
          <template #extra>
            <a-button v-if="selectedType" type="primary" size="small" @click="openCreateData">{{ t("dict.createData") }}</a-button>
          </template>
          ...
          <template v-else>
            <div style="margin-bottom: 12px;">
              <a-input-search v-model:value="dataKeyword" :placeholder="t('dict.searchDataPlaceholder')" allow-clear @search="loadData" />
            </div>
            <a-table
              :columns="dataColumns"
              ...
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'actions'">
                  <a-space>
                    ...
                    <a-popconfirm :title="t('dict.deleteDataConfirm')" @confirm="handleDeleteData(record.id)">
    ...
    <a-modal v-model:open="typeModalVisible" :title="typeEditTarget ? t('dict.editType') : t('dict.createType')" :confirm-loading="typeModalLoading" @ok="submitTypeForm" @cancel="closeTypeModal">
      <a-form ref="typeFormRef" :model="typeForm" layout="vertical" :rules="typeRules">
        <a-form-item :label="t('dict.typeCode')" name="code">
          <a-input v-model:value="typeForm.code" :disabled="!!typeEditTarget" :placeholder="t('dict.typeCodePlaceholder')" />
        </a-form-item>
        ...
        <a-form-item :label="t('dict.status')" name="status">
          <a-switch v-model:checked="typeForm.status" :checked-children="t('dict.statusEnabled')" :un-checked-children="t('dict.statusDisabled')" />
        </a-form-item>
    ...
    <a-modal v-model:open="dataModalVisible" :title="dataEditTarget ? t('dict.editData') : t('dict.createData')" :confirm-loading="dataModalLoading" @ok="submitDataForm" @cancel="closeDataModal">
      <a-form ref="dataFormRef" :model="dataForm" layout="vertical" :rules="dataRules">
        ...
        <a-form-item :label="t('dict.status')" name="status">
          <a-switch v-model:checked="dataForm.status" :checked-children="t('dict.statusEnabled')" :un-checked-children="t('dict.statusDisabled')" />
        </a-form-item>
...
import {
  getDictTypesPaged, createDictType, updateDictType, deleteDictType,
  getDictDataPaged, createDictData, updateDictData, deleteDictData
} from "@/services/api-dict";
import type { DictTypeDto, DictDataDto } from "@/services/api-dict";
...
async function loadTypes() {
  ...
  try {
    ...
    message.error(e instanceof Error ? e.message : t("dict.loadTypesFailed"));
  } finally {
...
const dataPagination = reactive<TablePaginationConfig>({ current: 1, pageSize: 20, total: 0, showSizeChanger: false });
...
async function loadData() {
  ...
  try {
    ...
    message.error(e instanceof Error ? e.message : t("dict.loadDataFailed"));
  } finally {
    dataLoading.value = false;
  }
}

function onDataTableChange(pag: TablePaginationConfig) {
  dataPagination.current = pag.current ?? 1;
  dataPagination.pageSize = pag.pageSize ?? 20;
  void loadData();
}
...
async function submitTypeForm() {
  ...
  try {
    if (typeEditTarget.value) {
      await updateDictType(typeEditTarget.value.id, { name: typeForm.name, status: typeForm.status, remark: typeForm.remark || undefined });
      message.success(t("dict.updateTypeSuccess"));
    } else {
      await createDictType({ code: typeForm.code, name: typeForm.name, status: typeForm.status, remark: typeForm.remark || undefined });
      message.success(t("dict.createTypeSuccess"));
    }
    typeModalVisible.value = false;
    void loadTypes();
  } catch (e: unknown) {
    message.error(e instanceof Error ? e.message : t("dict.operationFailed"));
  } finally {
...
async function handleDeleteType(id: string) {
  try {
    ...
    message.error(e instanceof Error ? e.message : t("dict.deleteTypeFailed"));
  }
}
...
const dataForm = reactive({ label: "", value: "", sortOrder: 0, status: true, listClass: undefined as string | undefined });
const dataRules = {
...
function openCreateData() {
  ...
  Object.assign(dataForm, { label: "", value: "", sortOrder: 0, status: true, listClass: undefined });
  dataModalVisible.value = true;
}

function openEditData(item: DictDataDto) {
  dataEditTarget.value = item;
  Object.assign(dataForm, { label: item.label, value: item.value, sortOrder: item.sortOrder, status: item.status, listClass: item.listClass || undefined });
  dataModalVisible.value = true;
}
...
async function submitDataForm() {
  ...
  try {
    const payload = { label: dataForm.label, value: dataForm.value, sortOrder: dataForm.sortOrder, status: dataForm.status, listClass: dataForm.listClass || undefined };
...
async function handleDeleteData(id: string) {
  try {
    ...
    message.error(e instanceof Error ? e.message : t("dict.deleteDataFailed"));
  }
}
