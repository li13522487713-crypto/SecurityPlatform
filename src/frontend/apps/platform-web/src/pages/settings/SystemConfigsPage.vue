...
async function loadConfigs() {
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : t("systemConfig.loadFailed"));
  } finally {
    loading.value = false;
  }
}
...
async function saveGroup(groupKey: string) {
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : t("systemConfig.operationFailed"));
  } finally {
    savingGroup.value = "";
  }
}
