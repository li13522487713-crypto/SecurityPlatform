    ...
    <template #form>
      <a-form ref="formRef" :model="formModel" :rules="formRules" layout="vertical">
        ...
        <a-form-item :label="t('systemMenus.menuType')" name="menuType">
          <a-select v-model:value="formModel.menuType" :options="menuTypeOptions" />
        </a-form-item>
...
import { createMenu, getMenusAll, getMenusPaged, updateMenu } from "@/services/api-users";
...
const selectedTreeKeys = computed(() =>
  selectedParentId.value != null ? [String(selectedParentId.value)] : []
);
...
const fetchData = async () => {
  ...
  loading.value = true;
  try {
    const isHidden = hiddenFilter.value === "all" ? undefined : hiddenFilter.value === "hidden";
...
const ensureParentOption = (parentId?: number) => {
  ...
  if (!exists) {
    parentOptions.value = [
      { label: t("systemMenus.menuIdLabel", { id: parentId }), value: parentId },
      ...parentOptions.value
    ];
  }
};
