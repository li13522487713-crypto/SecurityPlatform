...
/// <summary>
/// 应用草稿稿锁控制器（M04 S04-2，**设计态前缀** /api/v1/lowcode/apps/{id}/draft-lock）。
///
/// 设计：
/// - acquire / renew / release / status 由持有者 / 协作者使用。
/// - takeover 仅高权限（lowcode-app:update）可调用，强制夺锁后写审计。
/// - sessionId 由前端生成（uuid），与 M16 协同模式互斥。
/// </summary>
[ApiController]
...
public sealed class LowCodeAppDraftLockController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<AppDraftLockResult>>> Acquire(long appId, [FromBody] AcquireRequest req, CancellationToken cancellationToken)
    {
        ...
        var r = await _service.TryAcquireAsync(tenantId, appId, user.UserId, req.SessionId, cancellationToken);
        return Ok(ApiResponse<AppDraftLockResult>.Ok(r, HttpContext.TraceIdentifier));
    }
    ...
    {
        var tenantId = _tenantProvider.GetTenantId();
        var ok = await _service.RenewAsync(tenantId, appId, req.SessionId, cancellationToken);
        if (!ok) throw new BusinessException(ErrorCodes.Conflict, "锁已被他人夺取或不存在，请重新获取");
    ...
    public async Task<ActionResult<ApiResponse<AppDraftLockResult>>> Takeover(long appId, [FromBody] TakeoverRequest req, CancellationToken cancellationToken)
    {
        var tenantId = _tenantProvider.GetTenantId();
        var user = _currentUser.GetCurrentUserOrThrow();
        var r = await _service.ForceTakeoverAsync(tenantId, appId, user.UserId, req.SessionId, cancellationToken);
        return Ok(ApiResponse<AppDraftLockResult>.Ok(r, HttpContext.TraceIdentifier));
    }
