...
/// <summary>
/// 组件注册表控制器（设计态前缀 /api/v1/lowcode/components）。
...
public sealed class LowCodeComponentsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<ComponentRegistryDto>>> Registry([FromQuery] string? renderer, CancellationToken cancellationToken)
    {
        ...
        var result = await _service.GetRegistryAsync(tenantId, renderer, cancellationToken);
        return Ok(ApiResponse<ComponentRegistryDto>.Ok(result, HttpContext.TraceIdentifier));
    }
