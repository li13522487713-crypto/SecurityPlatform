...
/// <summary>
/// 应用模板控制器（设计态 v1 /api/v1/lowcode/templates）。
...
public sealed class LowCodeAppTemplatesController : ControllerBase
{
    ...
    {
        var tenantId = _tenantProvider.GetTenantId();
        var template = await _service.GetAsync(tenantId, id, cancellationToken)
            ?? throw new BusinessException(ErrorCodes.NotFound, $"模板不存在：{id}");
        return Ok(ApiResponse<AppTemplateDto>.Ok(template, HttpContext.TraceIdentifier));
    }
    ...
    public async Task<ActionResult<ApiResponse<AppTemplateApplyResult>>> Apply(long id, CancellationToken cancellationToken)
    {
        var tenantId = _tenantProvider.GetTenantId();
        var user = _currentUser.GetCurrentUserOrThrow();
        var result = await _service.ApplyAsync(tenantId, user.UserId, id, cancellationToken);
        return Ok(ApiResponse<AppTemplateApplyResult>.Ok(result, HttpContext.TraceIdentifier));
    }
