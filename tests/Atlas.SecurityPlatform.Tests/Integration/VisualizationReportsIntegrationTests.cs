...
public sealed class VisualizationReportsIntegrationTests
{
    private readonly HttpClient _client;

    public VisualizationReportsIntegrationTests(AtlasWebApplicationFactory factory)
    {
        _client = factory.CreateClient();
    }

    [Fact]
    public async Task QueryVisualizationOverview_ShouldReturnSuccess()
    {
        ...
        using var response = await _client.GetAsync("/api/v1/visualization/overview");
    ...
    [Fact]
    public async Task DeleteReportWithoutIdempotency_ShouldReturn400()
    {
        var accessToken = await IntegrationAuthHelper.LoginAndGetAccessTokenAsync(_client);
        var csrfToken = await CsrfIdempotencyHelper.GetAntiforgeryTokenAsync(_client, accessToken);

        using var request = new HttpRequestMessage(HttpMethod.Delete, "/api/v1/reports/1");
