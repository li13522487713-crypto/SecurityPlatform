...
- 禁止新增、扩展或保留“桥接层”形式的业务承载代码。
- 关键业务逻辑必须直接在本项目既有架构分层中实现（如 Controller/Service/Domain/Application/Infrastructure 等），不允许通过 shim/adapter/bridge 代替真实实现。
- 对外兼容或历史迁移场景如必须过渡，桥接仅允许做最小兼容映射，不得承载核心业务决策、权限、事务、验证等主链路逻辑。
- 发现桥接层承载主链路逻辑时，必须回归到原生架构实现并补齐对应契约与测试。

## 桥接层移除执行要求（硬规则）

- 本项目一旦发现桥接层（Bridge/Adapter/Facade Shim）承载真实业务链路，必须在同一任务内完成移除并回归重构。
- 处理顺序：先补齐真实分层实现 → 更新契约/DTO → 调整调用点 → 删除对应桥接层代码 → 补齐回归测试。
- 移除后不允许保留“兼容占位/空壳/旁路代理”式桥接代码。
- 仅允许的兼容映射为极小范围“非核心、无副作用、可移除”的边缘兼容层，不得用于主流程控制。
