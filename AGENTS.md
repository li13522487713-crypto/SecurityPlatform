...
- 对任何非平凡任务，AI 助理必须先判断是否需要并行只读探查，再决定是否进入实现阶段。
- 若满足以下任一条件，必须优先显式并行启动子代理做只读探查，再开始改代码：
  - 根因不明确
  - 问题可能跨越前端 / 后端 / 基础设施 / 测试
  - 涉及多个子系统、模块边界或宿主 / package 协作
  - 任务描述包含“排查 / investigate / root cause / trace / unexplained / intermittent / regression / cross-module”等信号词

### 默认并行探查扇出

1. `root_cause_explorer`
   - 追踪真实执行路径
   - 识别入口、调用链、配置开关、特性开关与受影响文件
   - 只读，不做代码修改
2. `repro_tester`
   - 复现问题
   - 检查日志、堆栈、失败测试与边界场景
   - 默认只读；除非主代理明确授权，否则不修改测试
3. `risk_reviewer`
   - 审查正确性、回归风险、安全影响与缺失校验
   - 只读，不做代码修改

### 主代理职责

- 等待所有子代理返回结果后再合并判断
- 汇总为一份简洁诊断，明确统一根因、影响范围与冲突点
- 若多个子代理结论冲突，必须先消解冲突，再进入实现
- 形成单一实现方案后，才允许开始代码修改

### 并行与串行规则

- 优先把并行子代理用于读多写少的工作：代码路径探索、日志分析、测试分流、配置排查、风险审查
