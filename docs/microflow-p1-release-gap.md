...
> 注：本文为历史快照（截至 P1 发布门禁阶段）。后续 P0-1 重构已将 `MicroflowApiControllerBase`
> 收敛为 `[Authorize]` + `MicroflowProductionGuardFilter` + `MicroflowWorkspaceOwnershipFilter`，
> 路由全部使用 `api/v1/...` 前缀。本文中关于 `[AllowAnonymous]` 与无 `v1` 路径的描述
> **仅作为历史背景**，最新真实状态请参阅 [`microflow-api-contract-current.md`](./microflow-api-contract-current.md)
> 与 [`microflow-production-refactor-gap-report.md`](./microflow-production-refactor-gap-report.md)。
