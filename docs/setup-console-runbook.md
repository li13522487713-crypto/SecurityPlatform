...
- M10：D1 Playwright 4 spec 全过 + 截图留痕；D2 SetupConsoleService 6 步 lifecycle 集成测试；D3 token 过期边界；D4 IP 限流（5 次/15 分钟）；D5 审计 IP/UA 自动注入（`SetupConsoleAuditContext` + middleware）；D6 mock API 下线；D7 平台菜单入口；D8 Repair Tab；D9 实体目录下钻；D10 上生产 checklist 文档
