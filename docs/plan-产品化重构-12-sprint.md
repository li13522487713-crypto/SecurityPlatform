...
- 明确并校验以下契约行为：
  ...
  - 跨租户访问拒绝
  - 敏感字段脱敏写入/返回
- 数据泄露防护主线并入平台封板前 P0/P1：
  - P0：分类分级、统一 DLP 策略、导出下载控制、文件外链控制、AI / 开放平台外发审计
  - P1：高风险动作审批、水印与追踪、知识库共享治理、Webhook / 连接器白名单、租户级例外策略
...
| Sprint | 核心目标 | 状态 |
|---|---|---|
| Sprint 1 | 重构基线冻结，统一元模型与领域边界定稿，接口命名与版本策略定稿，弃用清单发布，并冻结 DLP 元模型、五层控制模型与平台 / 租户 / 应用 / 运行四层责任 | [ ] |
| Sprint 2 | 后端骨架与数据库迁移（Manifest/Release/Policy/License/Package/DLP），仓储与服务层空实现及契约占位，纳入分类分级、标签、策略、通道、泄露事件、证据包 | [ ] |
| Sprint 3 | 平台控制面 V1（平台首页、应用中心、资源中心、DLP 治理入口）+ 对应聚合 API | [ ] |
| Sprint 4 | 发布中心与审计主链路（发布、回滚点、影响分析基础能力、高风险外发审计）+ 平台层 e2e | [ ] |
| Sprint 5 | 应用工作台 V1（概览、页面/表单、流程、数据、权限入口）+ 应用域 API | [x] |
| Sprint 6 | 统一设计器 V1（页面/表单/流程统一元数据存储），模板体系最小可用版本接入 | [ ] |
| Sprint 7 | 运行态 V1（`appKey/pageKey` 访问、任务/审批中心、发布态读取、导出 / 下载 / 文件 / API 调用前校验）+ 业务用户访问闭环 | [ ] |
| Sprint 8 | 流程闭环强化（审批流与工作流统一编排边界、状态回写、失败补偿、可观测性） | [ ] |
| Sprint 9 | 应用导入导出 + 导出下载控制 V1（结构包/基础数据包/完整副本三模式、审批、额度、频次、脱敏、水印、冲突检测与回滚） | [ ] |
| Sprint 10 | 离线 License 中心 V1（申请、签发、导入、校验、续期、审计），私有化场景验收 | [ ] |
| Sprint 11 | Tools Authorization Center V1（目录、策略矩阵、审批要求、限流配额、策略模拟、审计），并纳入 Connector / Webhook / AI 出站治理 | [ ] |
| Sprint 12 | 全链路硬化与发布门禁（Gate-R1/R2、性能与安全压测、弃用公告、上线文档与运维手册），并加入 DLP 场景验收 | [ ] |
...
- 集成测试：
  ...
  - 覆盖导出、下载、文件、Webhook、AI / 知识库出站的 DLP 策略链路
- 前端 e2e：
  - 三层入口切换
  - 发布与回滚
  - 业务用户运行态访问
...
- DLP 主线默认并入 12 Sprint 主计划，不另起平行排期。
