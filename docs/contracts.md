...
- `PlatformHost` 与 `AppHost` 额外提供 Coze intelligence 兼容入口：`/api/intelligence_api/*`
...
- 兼容层实现位于 [`CozeIntelligenceCompatControllerBase`](../src/backend/Atlas.Presentation.Shared/Controllers/Ai/CozeIntelligenceCompatControllerBase.cs)，由 `Atlas.PlatformHost` 与 `Atlas.AppHost` 两侧控制器共同挂载。
...
- 全部 `/api/workflow_api/*`、`/api/intelligence_api/*`、`/api/playground_api/*`、`/api/draftbot/*`、`/api/bot/*`、`/v1/workflows/*`、`/v1/workflow/*` 路由均挂 `[Authorize]`，由 JWT 中间件强制认证（PAT 走 `PatAuthenticationHandler`）。
