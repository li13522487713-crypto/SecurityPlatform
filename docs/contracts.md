...
- `WorkspaceWorkflowWorkbenchRoute` 当前默认直接走 cozelib 工作流编辑器；`@coze-studio/workspace-adapter` 的 `develop`/`library` 与 `@coze-agent-ide/*` 的原生入口后续继续按覆盖矩阵补齐接口后再分批切换，不再依赖 `@coze-arch/bot-api` shim。
