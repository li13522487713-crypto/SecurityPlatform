...
| 机器人（Bot，Coze 兼容） | 仅出现在 Coze 原生兼容层 / intelligence 兼容层 | `/api/bot/*`、`/api/draftbot/*`、`/v1/bots/*` | 不在 Atlas 域模型中再造同名实体 |
| 智能体技能装配 | `AgentPluginBinding` / `AgentKnowledgeLink` / `AgentWorkflowBinding`(`Skill` 角色) / `AgentDatabaseBinding` / `AgentVariableBinding` / `AgentPromptBinding` / `AgentConversationProfile` | 同 agents 子路由 | `AgentPromptBinding` 当前为预留绑定模型；提示模板不再作为 `Agent` 主表字段持久化 |
...
- 新前端主入口统一收敛为 `/workspace/*` 与平台公共页：
  ...
  - `/workspace/:workspaceId/evaluations`
  - `/workspace/:workspaceId/settings/publish`
  ...
  - `/open/api`
  - `/docs`
- 历史壳路由 `/w/*`、`/workspaces`、`/org/:orgId/workspaces/*`、`/apps/:appKey/*` 已下线，不再提供兼容跳转窗口。
...
- 当前工作流原生入口统一由 `AppHost` 承载：`/api/workflow_api/*`。
- `app-web` 工作流主链直接消费 `@coze-workflow/playground-adapter -> @coze-workflow/* -> /api/workflow_api/*`，不再依赖 `@coze-arch/bot-api` shim。
- `api/app-web/workflow-sdk/*` 仍保留为历史兼容资产，但不再作为 Coze 原生主契约维护。
- 当前已覆盖 workflow playground 必需接口：
  - `canvas`
  - `save`
  - `publish`
  - `node_type`
 ...
 - 当前主链路已切到 `app-web -> /api/workflow_api/* -> CozeWorkflow*`；旧 `/api/v2/workflows*` 已下线。

## Coze Developer / Playground API

- 当前 developer / playground 原生入口统一由 `AppHost` 承载：
  - `/api/playground_api/*`
  - `/api/bot/*`
  - `/api/space/*`
  - `/api/draftbot/*`
...
- 第一批原生直连已接真实服务：
  ...
  - `GET/POST /v1/bots*`
- 第一批接口覆盖矩阵以 [`docs/coze-interface-coverage-matrix.md`](coze-interface-coverage-matrix.md) 为准；凡标记 `Partial` / `Missing` 的端点，不得在删除兼容层后视为完成。

## Coze Passport API

- `foundation-sdk` 现行原生鉴权入口统一由 `AppHost` 承载：
...
- `app-web` 已移除 `@coze-foundation/foundation-sdk -> @atlas/foundation-bridge` alias；当前 foundation 相关登录态、space store 与用户态读取直接走原生 Coze 包与上述接口。

## Coze Intelligence API 兼容层

- 当前开发态 Coze intelligence 兼容入口统一由 `AppHost` 承载：`/api/intelligence_api/*`。
- 当前已覆盖开发列表 / 项目 IDE 高频依赖端点：
...
- Atlas 当前未落完整 Coze intelligence 域模型时，上述接口按“稳定占位兼容”返回可消费结构：
  - `get_draft_intelligence_list` 返回空 `intelligences[]`、`total=0`、`has_more=false`
  - `get_draft_intelligence_info` 返回基于 `intelligence_id` / `intelligence_type` 组装的占位 `basic_info`、`publish_info`、`owner_info`
  - `draft_project/*` 与 `entity_task/*` 返回成功结构，确保前端页面、轮询与重试链路不再因 404 中断
- 兼容层实现位于 [`CozeIntelligenceCompatControllerBase`](../src/backend/Atlas.Presentation.Shared/Controllers/Ai/CozeIntelligenceCompatControllerBase.cs)；当前运行默认以 `Atlas.AppHost` 为唯一有效宿主。

### Coze 兼容层风险已修复登记

| 风险 | 状态 | 修复点 | 回归用例 |
|---|---|---|---|
| 1. 不重写上游 packages/workflow/* / packages/coze-* | 已遵守 | 本次收口仍未改写 `packages/workflow/*`、`packages/agent-ide/*`、`packages/foundation/*`、`packages/arch/*` 的上游实现；宿主改动集中在 `app-web` 接线与 AppHost 原生路由补齐。 | n/a（约束类） |
| 2. 不引入新依赖 | 已遵守 | 本次改动未新增第三方依赖；只删除 `@atlas/foundation-bridge` 与 `coze-shims`，并改为直连现有 workspace 内 `@coze-*` 包。 | n/a（约束类） |
...
| 5. foundation-sdk 行为差异 | 已收口 | `app-web` 已移除 `atlas-foundation-bridge`，改为通过原生 `/api/passport/*`、`/api/playground_api/*`、`/api/space/*` 与真实 `@coze-foundation/foundation-sdk` 对接。 | `src/backend/Atlas.AppHost/Bosch.http/CozeNativeDirectRoutes.http` |
...
- `app-web` 工作流主链路当前以 Coze SDK + `/api/workflow_api/*` 为唯一有效入口；旧 `DagWorkflow*` 与 `/api/v2/workflows*` 已退场，不再作为主契约维护。
...
- `POST /api/workflow_api/create` 直接调用 `ICozeWorkflowCommandService.CreateAsync`；
...
- `POST /api/playground_api/space/info` 与 `POST /api/space/info`：均走 `IWorkspacePortalService.ListWorkspacesAsync`，返回 `{data: {id, name, description, icon_url, space_type, role_type, space_mode}}`。

### M5 等保 / 多租户 / 鉴权

- 全部 `/api/workflow_api/*`、`/api/playground_api/*`、`/api/bot/*`、`/api/space/*`、`/api/draftbot/*`、`/api/passport/*`、`/api/web/user/*`、`/api/user/*`、`/api/intelligence_api/*`、`/v1/workspaces/*`、`/v1/bots/*`、`/v1/workflows/*`、`/v1/workflow/*` 路由均挂 `[Authorize]`，由 `AppHost` JWT 中间件强制认证（PAT 走 `PatAuthenticationHandler`）。
...
- `WorkspaceWorkflowWorkbenchRoute` 当前默认直接走 cozelib 工作流编辑器；workspace 的 `develop` / `library` 入口已切到 `@coze-studio/workspace-adapter`，`@coze-agent-ide/*` 等其余原生入口继续按覆盖矩阵分批切换，不再依赖 `@coze-arch/bot-api` shim。
...
## Workflow 历史 V2 契约（已下线，仅供迁移参考）

> 当前有效主链路：
> - 编辑态：`app-web -> /api/workflow_api/*`
> - 运行态：`/api/runtime/workflows/{id}:invoke`、`/api/runtime/workflows/{id}:invoke-async`、`/api/runtime/workflows/{id}:invoke-batch`
> - 下述 `/api/v2/workflows*` 内容仅保留为历史迁移说明，不再视为现行主契约。
...
- `AppHost` 是唯一权威宿主；`app-web` 默认命中 `AppHost`。
- `app-web` 是低代码设计器唯一受支持宿主；`/api/v1/lowcode/**` 统一命中 `AppHost (5002)`。
- 插件市场前端统一消费：
...
- `app-web` 前端收口路由：
  - `/market/plugins`（插件列表入口）
  - `/market/templates`（模板列表入口）
  - `/explore/plugin/:productId`（插件详情）
  - `/explore/template/:templateId`（模板详情）

### AI 数据库补充契约

- **元数据**：`GET /api/v1/ai-databases/{id}` 返回 `tableSchema`（仅用户字段 JSON）、`fields[]`（含系统列 id / sys_platform / uuid / bstudio_create_time）、`channelConfigs[]`（与 `ChannelCatalog` 对齐的默认或已保存配置）。
- **模式**：`PUT /api/v1/ai-databases/{id}/mode` 请求体 `queryMode`（`0` MultiUser / `1` SingleUser）与 `channelScope`（`0` FullShared / `1` ChannelIsolated / `2` InternalShared）。
- **渠道读写**：`GET|PUT /api/v1/ai-databases/{id}/channel-config`；写入时 `items[]` 含 `channelKey`、`allowDraft`、`allowOnline` 等。运行时若带 `X-App-Channel` 且对应项关闭，则拒绝写入对应 environment。
- **物理表**：每张库两张 SQLite 表（draft/online），详见 `docs/plan-ai-database.md`。
...
> 由 `KnowledgeBasesV5Controller` 挂在 `AppHost` 的 `api/v1/knowledge-bases` 路由树下。
...
- `AppHost` 是统一权威宿主，承载工作台 / 资源库 / 市场 / 设计态 / 发布态 / PAT / OpenAPI 项目管理、对话 / Agent 调试 / Workflow 运行 / Embed Chat / OpenAPI 运行。
...
- 请求 / 响应结构以 `AppHost` 的 `ai-assistants` 契约为准，用于 `app-web` 直连模式。
...
- `GET /api/v1/ai-apps/{id}/builder-config`（AppHost）
- `PUT /api/v1/ai-apps/{id}/builder-config`（AppHost）
...
### Workflow 依赖查询（AppHost）

- `GET /api/v2/workflows/{id}/dependencies`
- 当前开发态唯一有效宿主为 `AppHost`。
...
- `AgentDetail` / `AgentCreateRequest` / `AgentUpdateRequest` 扩展：
  - 提示模板不再通过 `Agent.PromptTemplateId` 暴露；若后续恢复提示模板绑定能力，统一走 `AgentPromptBinding`
  - `knowledgeBindings[]`
    - `knowledgeBaseId`
    - `isEnabled`
    - `invokeMode`
...
- `POST /api/v1/workspace-ide/resources/{resourceType}/{resourceId}/duplicate`
  - 请求体：`WorkspaceIdeResourceDuplicateRequest`
    - `workspaceId`
    - `folderId?`
- `POST /api/v1/workspace-ide/resources/{resourceType}/{resourceId}/migrate`
  - 请求体：`WorkspaceIdeResourceMoveWorkspaceRequest`
    - `sourceWorkspaceId`
    - `targetWorkspaceId`
- `POST /api/v1/workspace-ide/resources/{resourceType}/{resourceId}/copy-to-workspace`
  - 请求体：`WorkspaceIdeResourceMoveWorkspaceRequest`
    - `sourceWorkspaceId`
    - `targetWorkspaceId`
- `DELETE /api/v1/workspace-ide/resources/{resourceType}/{resourceId}`
- 动作返回（duplicate/migrate/copy）：
  - `ApiResponse<WorkspaceIdeResourceActionResult>`
  - `resourceType`
  - `resourceId`
  - `action`
  - `workspaceId?`
...
> 草案 → 已落地。前端 mock API 已下线并统一走后端 HTTP。
> 后台实现位于 `Atlas.AppHost/Controllers/{HomeContent,Community,PlatformGeneral,MarketSummary,MeSettings}Controller.cs`，
...
| 后台 Controller | 前端 client |
|---|---|
| `HomeContentController` | `services/api-home-content.ts` |
| `CommunityController` | `services/api-community.ts` |
| `PlatformGeneralController` | `services/api-dashboard-stats.ts` / `services/api-home-content.ts` 等真实 HTTP client |
...
> 注：在 AppHost 内运行时接口统一使用 `/api/runtime/...`；历史 `/api/v1/runtime/...` 兼容写法不再作为新契约入口。
...
> 2026-05-01：前端 mock API 目录已下线，上述路由均通过 `AppHost` 真实 HTTP API 承载。
...
`status` 在 JSON 中序列化为整数（0/1/2/3）。前端真实 client 负责 `number → string` 兼容映射，
...
| 后台 Controller | 前端 client |
|---|---|
| `WorkspaceTasksController` | `services/api-workspace-runtime.ts` |
| `WorkspaceEvaluationsController`（含 testsets 子路径） | `services/api-workspace-runtime.ts` |
...
>
> 本章为后端最终契约；前端通过 `src/frontend/apps/app-web/src/services/api-setup-console.ts` 连接真实 HTTP API，不再保留 mock API。
...
- 权威宿主：`Atlas.AppHost (5002)`；setup-console / migration / tenant-datasources 运行时逻辑仅由 AppHost 承载。
...
> | 前缀 | Host | 端口 | 用途 |
> | --- | --- | --- | --- |
> | `/api/v1/lowcode/*` | AppHost | 5002 | 设计态写操作（schema / pages / variables / versions / publish / templates / faq / prompt-templates / plugins / overrides 等） |
...
> - `/hubs/lowcode-preview`（M08，AppHost 挂载，HMR 推送由设计态写入服务直接调用 `ILowCodePreviewSignal` 触发）
...
### app-web 内打开 Lowcode Studio

- `app-web` 中“项目开发 -> 点应用打开”统一使用站内路由 `/apps/lowcode/:appId/studio`，不再依赖独立 `172.18.0.1:5183` studio 端口。
- `src/frontend/apps/lowcode-studio-web` 进入废弃过渡阶段，仅保留兼容调试入口；功能正确性以 `app-web` 内嵌低代码设计器为准。
- 低代码设计器壳层通过 `@atlas/lowcode-studio-react` 共享包复用，但设计态请求、鉴权与运行模式分发必须由 `app-web` 宿主注入；禁止 `apps/app-web` 直接跨 apps 引用 `apps/lowcode-studio-web/src/**`。
...
接入控制器：`DagWorkflowController` / `AiAppsController` / `KnowledgeBasesController` / `AiDatabasesController`（AppHost 权威实现）的 Update / Delete / Publish / Run 等写动作前置 `GuardByResourceAsync(...)`，写后 `InvalidateAsync(...)` 清 PDP 资源 tag。资源类型常量：`workflow` / `app` / `knowledge` / `database`。
...
> 前端包：`@atlas/external-connectors-react`；app-web 统一挂载到工作区设置路由 `/workspace/:workspaceId/settings/*`（旧 `/org/:orgId/workspaces/:workspaceId/settings/connectors/*` 已下线）。
>
> **NuGet 集成（N1-N3 替代自研）**：
...
- Runtime FlowNavigator 诊断能力：`POST /api/v1/microflows/runtime/navigate` 与 `GET /api/v1/microflows/{id}/runtime/navigate` 仅消费 `MicroflowExecutionPlan` 做控制流 dry-run，输出 `MicroflowNavigationResult` / trace skeleton，不执行真实 action、表达式、数据库 CRUD、REST 或事务。前端 mock runner 已下线，TestRun 必须走后端 runtime API。
...
- 后端第 53 轮新增 Runtime TransactionManager / UnitOfWork：事务只作用于 `RuntimeExecutionContext`，维护内存 change set、savepoint、transaction log、TraceFrame `output.transaction`、RuntimeLog `transaction.*` 与 RunSession `transactionSummary`；不访问业务数据库、不调用 ORM SaveChanges、不执行真实 CRUD/REST/Object events。
...
- Microflow Workbench P0/P1 运行/调试闭环约束：
  ...
  - 前端 AuthoringSchema 在保存、服务端校验和试运行前统一执行 `normalizeMicroflowAuthoringSchemaForRuntime`：补齐 schemaVersion / loop collection id，修复 Boolean Decision 空 `caseValues`，把同一 loop body 内误落 root 的 flow 归回 loop collection，检查重复 node id / flow id，并对非法跨 collection sequence flow 输出 blocking issue；后端 Validator 仍保持严格拒绝非法 flow，不做兼容放宽。
  - Normalizer 与后端 Validator 对重复 object id / flow id 必须统一输出 `MF_OBJECT_ID_DUPLICATED` / `MF_FLOW_ID_DUPLICATED` blocking issue，并带 `objectId` / `flowId` / `fieldPath`；重复 flow id 不得进入 repair map 覆盖另一条 flow，重复 object id 不得导致后端 Validator 构建对象索引时抛异常，必须保留原始非法结构供 Problems 面板和后端校验定位。
  - Loop Body 入口是唯一允许跨 root / loop collection 的建模边界，必须由 LoopedActivity 的 `loopBodyIn` 端口生成 `edgeKind=loopBody`，并把 flow 存入目标 loop objectCollection；Loop Body 返回不得生成跨 collection flow，必须通过 Break / Continue Runtime 语义表达。
  - 前后端 ExecutionPlan 都必须保留 `edgeKind=loopBody`，但把该 flow 标记为 `controlFlow=ignored` 并放入 `ignoredFlows`；root 主流程只能通过 LoopedActivity 节点本身继续，不得把 Loop Body 入口作为普通 `normalFlow` 导航。
  - AnnotationFlow 必须同样保留在 `flows` / `ignoredFlows` 中，且 `controlFlow=ignored`、`runtimeIgnored=true`；不得进入 `normalFlows`、`decisionFlows`、`objectTypeFlows` 或 `errorHandlerFlows`。
  - Boolean Decision 缺少 `true` / `false` 分支时，前后端校验 issue 必须携带 Decision `objectId`、`fieldPath=*.splitCondition`、已有 outgoing `relatedFlowIds`。后端 `MicroflowValidationIssueDto.quickFixes[]` 需给出 `createMissingFlow` patch 元数据；前端 Problems 面板可基于同一 issue 一键创建缺失 boolean case 的分支线，root collection 内创建 End Event 目标，loop body 内创建 Continue Event 目标，且不得生成跨 collection flow。
  ...
  - `MicroflowTraceFrameDto` / 前端 `MicroflowTraceFrame` 统一 Node I/O 字段：`nodeKind`、`actionKind`、`inputVariables`、`actionInput`、`evaluatedExpressions`、`outputVariables`、`outputMappingsResolved`、`variableDelta`、`handoffPayload`、`transactionEffect`。`outputMappingsResolved` 是按节点 `data.outputMappings[]` 解析后的最终输出字段集合。上述字段通过 `GET /api/v1/microflows/runs/{runId}/trace` 回读，允许前端 Debug 面板按节点展示输入、输出、变量 delta、selected case、loop iteration、handoff 与事务摘要；大对象仍按 runtime trace 摘要化策略处理。
  - 前端 Debug 面板必须通过纯 `buildMicroflowNodeIoViewModel` 组装 Node I/O 分组，至少覆盖 summary、input、output、flow、runtime 五组数据，避免 UI 组件渲染逻辑与 trace 契约字段漂移。
  - Action frame 的 `actionInput` 必须包含 `nodeKind`、`actionKind`、`actionId` 与脱敏后的 `actionConfig`，敏感键（password / secret / token / authorization / apiKey / credential）统一遮罩；`evaluatedExpressions` 必须递归识别 input/output 中嵌套的 `*expression*` 字段，供 Debug 面板定位表达式来源。
  - 前端表达式编辑器必须保留变量/属性 completion，并提供复合条件快捷 token（`and` / `or` / `not`、括号、比较运算符），所有输入仍交由 Microflow Expression DSL validator 做语法与类型诊断，禁止引入任意 JS/C# 执行路径。
  - Runtime trace 的 `input` / `output` 即使在 TestRun 或 Debug Full capture 模式下也必须受 `ExecutionMemoryBudget.MaxNodeOutputBytes` 约束；超限 payload 统一返回 `{ summary, sizeBytes, preview }`，禁止大节点输出绕过预算完整写入 trace。
  - Parallel / Inclusive Gateway 的 trace output 必须包含 `branchTrace[]`，每项携带 `flowId`、`branchId`、`targetObjectId`、`selected`、`status`；Parallel 分支成功后标记 `completed`，Inclusive 未选中分支必须显式标记 `skipped`，供 Debug 面板展示 executed / skipped / failed 分支状态。
  - 前端节点工厂和 node registry 必须把 `parallelGateway` / `inclusiveGateway` 创建为真实 Gateway object kind，禁止退化为 `actionActivity` 或 LogMessage；Inclusive Gateway 出边可在普通 sequence flow 上携带 expression / fallback caseValues，供 Runtime 选择多分支。
  - 前端 `MF_AllNodeComplexComputation_Test` 验收微流生成器必须沉淀为无 UI 依赖 fixture，至少单测覆盖 Boolean Decision true/false、Loop Body `ignoredFlows`、ObjectType flow、Parallel/Inclusive Gateway descriptors 和 `sortList` / `filterList` / `cast` P0 action runtime DTO 映射；编辑器一键生成入口必须复用该 fixture。
  - `sortList` / `filterList` / `cast` 的 `p0ActionRuntime` 映射必须有独立单元测试覆盖字段别名归一化，不能只依赖整体验收 fixture 间接覆盖。
  - 前端 P0 ExecutionPlan 映射必须覆盖 Runtime 已支持的列表/对象关键动作，包括 `sortList`、`filterList`、`cast`，避免 supported action 编译后缺失 `p0ActionRuntime`。
  - `MicroflowExecutionPlan.gateways[]` 必须显式编译 Parallel / Inclusive Gateway 节点的 `role`、`incomingFlowIds`、`outgoingFlowIds` 与 `branchFlowIds`；`role` 取值为 `split` / `merge` / `splitMerge` / `passThrough`，供 Runtime、FlowNavigator 与 Debug 面板共享同一网关结构语义。
  - Runtime 执行 Gateway fork/join 时优先使用 `ExecutionPlan.gateways[]` 中 `merge` / `splitMerge` 角色定位 join 节点；仅当 plan 未提供可用 gateway 描述时，才回退到基于 RuntimeGraph 距离的兼容推断。
  - ExecutionPlan Validator 必须校验 `gateways[]` 引用的 node / flow 存在性、gateway kind 合法性、split 至少两条 `branchFlowIds`、merge 至少一条 `incomingFlowIds`；相关错误码包括 `RUNTIME_GATEWAY_NODE_NOT_FOUND`、`RUNTIME_GATEWAY_FLOW_NOT_FOUND`、`RUNTIME_GATEWAY_KIND_INVALID`、`RUNTIME_GATEWAY_SPLIT_BRANCH_MISSING`、`RUNTIME_GATEWAY_MERGE_INCOMING_MISSING`。
  - Gateway 分支变量使用 `DefaultVariableScopeForker` 隔离执行，`GatewayWriteConflictDetector` 在合并前拒绝多分支写同名变量或同对象成员；`DefaultBranchMergePolicy` 只合并分支实际写过的变量，未写入的只读拷贝不得泄漏回父作用域，合并输出变量标记为 `branchMerge`。
...
- P0-7 补齐 Microflow 资源 API：
  ...
  - `GET /api/v1/microflows/{id}/callees` 列出"我调用了哪些下游"，从 reference index 中按 `sourceType=microflow + sourceId={id}` 过滤。
  - 上述端点统一支持 `includeInactive=false&sourceType=&impactLevel=` query 过滤。
