...
- 前端与后端协议当前统一收敛到 `AppHost` 的 app-web gateway：
  - workflow：`/api/app-web/workflow-sdk/*`
  - developer：`/api/app-web/coze-developer/*`
  - playground：`/api/app-web/coze-playground/*`
  - intelligence：`/api/intelligence_api/*`（当前仍保留独立兼容入口）
...
| 机器人（Bot，Coze 兼容） | 仅出现在 app-web gateway / intelligence 兼容层 | `api/app-web/coze-developer/*`、`api/app-web/coze-playground/*` | 不在 Atlas 域模型中再造同名实体 |
...
## Coze Workflow Gateway

- 当前工作流现行入口统一由 `AppHost` 承载：`/api/app-web/workflow-sdk/*`。
- 旧 `/api/workflow_api/*`、`/api/op_workflow/*` 运行期入口已删除，不再作为主契约维护。
- gateway 当前覆盖 workflow playground 必需接口：
  - `canvas`
  - `save`
  - `publish`
  - `node_type`
...
- 当前主链路已切到 `app-web -> /api/app-web/workflow-sdk/* -> CozeWorkflow*`；旧 `/api/v2/workflows*` 与 `/api/workflow_api/*` 均已下线。

## Coze Developer / Playground Gateway

- 当前 developer / playground 现行入口统一由 `AppHost` 承载：
  - `/api/app-web/coze-developer/*`
  - `/api/app-web/coze-playground/*`
- 旧 `/api/playground_api/*`、`/api/bot/*`、`/api/space/*`、`/api/draftbot/*` 运行期入口已删除，前端通过 app-web shim 自动重写到上述新 gateway。
- 现状约束：
  - 高频能力已接真实服务：workspace/space 列表、space info、type list、draftbot list、draftbot display info、open workspace/member 基础操作。
  - 长尾能力仍允许由新 gateway 返回 fallback 兼容结构，保证页面不因 404/500 中断；这些端点不应被视为“已接真实 Atlas 服务”。

## Coze Intelligence API 兼容层

- 当前开发态 Coze intelligence 兼容入口统一由 `AppHost` 承载：`/api/intelligence_api/*`；`PlatformHost` 目录仅保留历史兼容实现参考。
...
- 兼容层实现位于 [`CozeIntelligenceCompatControllerBase`](../src/backend/Atlas.Presentation.Shared/Controllers/Ai/CozeIntelligenceCompatControllerBase.cs)；当前运行默认以 `Atlas.AppHost` 为唯一有效宿主，`Atlas.PlatformHost` 控制器保留为历史兼容资产。
...
- `app-web` 工作流主链路当前以 Coze SDK + `api/app-web/workflow-sdk/*` 为唯一有效入口；旧 `DagWorkflow*` 与 `/api/v2/workflows*` 已退场，不再作为主契约维护。
...
- `POST /api/app-web/workflow-sdk/create` 直接调用 `ICozeWorkflowCommandService.CreateAsync`；
...
- `POST /api/app-web/coze-playground/space/info`：走 `IWorkspacePortalService.ListWorkspacesAsync`，返回 `{data: {id, name, description, icon_url, space_type, role_type, space_mode}}`。

### M5 等保 / 多租户 / 鉴权

- 全部 `api/app-web/workflow-sdk/*`、`api/app-web/coze-developer/*`、`api/app-web/coze-playground/*`、`/api/intelligence_api/*`、`/v1/workflows/*`、`/v1/workflow/*` 路由均挂 `[Authorize]`，由 `AppHost` JWT 中间件强制认证（PAT 走 `PatAuthenticationHandler`）。
...
|---|---|---|
| `POST /api/app-web/workflow-sdk/validate_tree` | `WorkflowService.ValidateTree`（`workflow_svc.thrift`） | Atlas 暂不支持 `bind_project_id` / `bind_bot_id` 真实校验，仅做透传与 schema 结构性校验；`schema` 为空且传 `workflow_id` 时回退使用当前草稿。 |
| `POST /api/app-web/workflow-sdk/node_panel_search` | `WorkflowService.NodePanelSearch` | Atlas 节点目录与上游 `NodePanelPlugin` 不同源，命中条目以 Coze `Workflow` 结构返回 `workflow_id = node_type_code`、`node_type = node_type_code`、`category = WorkflowNodeMetadata.Category`，前端需要据此渲染为节点条目。`page_or_cursor` 视为页码字符串。 |
| `POST /api/app-web/workflow-sdk/history_schema` | `WorkflowService.GetHistorySchema` | 优先级：`execute_id` → `commit_id` → 最新版本 → 当前草稿；`flow_mode/bind_biz_*` 字段为占位值；`commit_id` 同时接受 `WorkflowVersion.Id` 与 `version_number` 两种字符串形式。 |
| `GET /api/app-web/workflow-sdk/get_node_execute_history` | `WorkflowService.GetNodeExecuteHistory` | 字段命名沿用上游 `NodeResult`（`nodeId`、`NodeType` 等大小写）；`extra` 字段在 Atlas 实现中是 JSON 字符串，结构为 `{"input": <inputJson>, "output": <outputJson>, "variables": <ctxSnapshotJson>}`，便于调试面板直接消费上下文变量快照。 |
| `POST /api/app-web/workflow-sdk/get_trace` | `WorkflowService.GetTraceSDK` | spans 在 Atlas 中按 `step.StartedAt` 升序排列，并在头部追加根 span（`is_entry=true`、`type=Workflow`）；`spans[].extra.variables` 为「该步骤完成前累积的执行上下文」JSON 字符串，前端调试面板用于渲染变量快照。 |
...
> 当前有效主链路：
> - 编辑态：`app-web -> /api/app-web/workflow-sdk/*`
...
- `app-web` 是低代码设计器唯一受支持宿主。`platform` 模式下 `/api/v1/lowcode/**` 命中 `PlatformHost (5001)`；`direct` 模式下命中 `AppHost (5002)`。
- 插件市场前端统一消费：
...
- 当前开发态唯一有效宿主为 `AppHost`；`PlatformHost` 同名接口仅保留历史兼容资产，不再作为主契约。
