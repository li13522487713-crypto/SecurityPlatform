...
- `src/frontend/apps/lowcode-studio-web` 仍保留独立开发入口；`pnpm -C src/frontend run dev:lowcode-studio` 仅用于单独调试 lowcode 子应用，不是 app-web 正常打开应用的前置条件。
- 低代码设计器壳层通过共享包复用，`app-web` 与 `lowcode-studio-web` 必须消费同一套 studio 壳；禁止 `apps/app-web` 直接跨 apps 引用 `apps/lowcode-studio-web/src/**`。
