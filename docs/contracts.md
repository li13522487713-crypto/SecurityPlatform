...
- 前端与后端协议对齐统一走兼容层：`/api/workflow_api/*`、`/api/intelligence_api/*`、`/api/playground_api/*`、`/api/op_workflow/*`、`/api/bot/*`、`/api/space/*`、`/api/draftbot/*`、`/v1/workflow/*`、`/v1/workflows/*`。
...
- 当前开发态原生 Coze 协议兼容入口统一由 `AppHost` 承载：`/api/workflow_api/*`；`PlatformHost` 目录仅保留历史兼容实现参考。
...
- 当前主链路已切到 `app-web -> /api/app-web/workflow-sdk/* -> CozeWorkflow*`；旧 `/api/v2/workflows*` 已下线，`/api/workflow_api/*` 仅保留历史兼容，不再作为主契约维护。
...
- 全部 `/api/workflow_api/*`、`/api/intelligence_api/*`、`/api/playground_api/*`、`/api/draftbot/*`、`/api/bot/*`、`/v1/workflows/*`、`/v1/workflow/*` 路由均挂 `[Authorize]`，由 `AppHost` JWT 中间件强制认证（PAT 走 `PatAuthenticationHandler`）。
