...
- 后端第 49 轮已新增 Runtime FlowNavigator 诊断能力：`POST /api/v1/microflows/runtime/navigate` 与 `GET /api/v1/microflows/{id}/runtime/navigate` 仅消费 `MicroflowExecutionPlan` 做控制流 dry-run，输出 `MicroflowNavigationResult` / trace skeleton，不执行真实 action、表达式、数据库 CRUD、REST 或事务，不替代既有 TestRun Mock API。
