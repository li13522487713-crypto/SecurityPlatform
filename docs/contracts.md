...
- `GET /api/v1/dag-workflows/node-types` 返回的每个节点新增 `formMetaJson` 字段（JSON 数组，描述属性面板字段：`key/label/type/required/...`），仅 KB / DB / NL2SQL 节点提供，其余为 `null`。
...
- ActivitySource：`Atlas.AiPlatform.WorkflowNodes`，节点级 span 名称：
  - `AiDatabase.Insert / Update / Delete / Query`
  - `Knowledge.Retrieve / Index / Delete`
- 等保审计：每个写节点写入 `IAuditWriter`，action 形如 `ai_database_node.insert` / `knowledge_node.retrieve`，target 含 `db:/kb:/node:` 标识。
