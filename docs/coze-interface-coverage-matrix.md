...
| developer / publish | `@coze-arch/bot-space-api` | SpaceApi | `PublishConnectorList` | `POST /api/draftbot/publish/connector/list` | `space_id`, `bot_id` | `commit_version` | `publish_connector_list[]`, `connector_brand_info_map`, `publish_tips` | Partial | 是 | 部分 | 是 | 否 |
