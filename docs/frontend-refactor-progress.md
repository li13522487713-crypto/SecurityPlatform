...
| **B. 功能补齐 12 Case** | `plan-功能补齐总览.md` | 🟢 12/12 已勾选 | 🔴 核心 |
| **C. 产品化重构 12 Sprint** | `plan-产品化重构-12-sprint.md` | 🟡 Sprint 5 已勾选，其余待推进 | 🟡 中等 |
...
| # | 整改项 | 文档状态 | 代码实际状态 | 结论 |
|---|---|---|---|---|
| P0-1 | 清理/替换 `innerHTML` 注入点，使用 DOMPurify 或安全 DOM API | ✅ 已完成 | ✅ **已完成** — `MarkdownRenderer.vue` 已接入 DOMPurify 白名单净化并限制危险协议；`AmisEditor.vue` / `amis-renderer.vue` 已移除 `innerHTML` 清空容器写法，改为安全 DOM API | ✅ **已完成** |
...
| P0-3 | 手写业务代码中限制新增 `any`（先从 Workflow/Approval 开始） | 🟡 进行中 | 🟡 **主链路已收口** — Workflow/Approval 及关键页面首批 `any` 已治理（含 `WorkflowDesignerPage.vue`、`WorkflowEditorPage.vue`、`WorkflowInstancesPage.vue` 等），其余存量进入后续迭代 | 🟡 **持续推进** |
...
|---|---|---|---|---|---|---|
| Case 01 | 认证安全基线 | ✅ | ✅ | ✅ | ✅ | ✅ 已在总览文档标记完成，前端链路含 CSRF/幂等/Cookie 与登录安全基线 |
| Case 02 | 动态表 CRUD | ✅ | ✅ | ✅ | ✅ | ✅ 已完成全链路联调（含应用工作台数据页子路由与 AMIS 跳转） |
| Case 03 | 审批流设计运行 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成路由与通知待办深链闭环 |
| Case 04 | 定时任务管理 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成 Hangfire 相关前后端联调 |
| Case 05 | 服务监控健康 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成监控指标展示联调 |
| Case 06 | 项目域权限隔离 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成项目上下文切换、自动刷新与请求校验口径统一 |
| Case 07 | 多数据源接入 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成租户边界过滤、已保存数据源测试与测试延迟展示 |
| Case 08 | 自定义表格视图 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成表格视图前后端联调 |
| Case 09 | Excel 导入导出 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成导入导出与错误反馈闭环 |
| Case 10 | 数据权限 DataScope | ✅ | ✅ | ✅ | ✅ | ✅ 已完成角色配置入口、作用域预览与联调说明补齐 |
| Case 11 | 通知公告闭环 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成 /inbox、撤回、阅读审计与权限口径修正 |
| Case 12 | 生产级安全加固 | ✅ | ✅ | ✅ | ✅ | ✅ 已完成 Markdown 协议白名单、`innerHTML` 清理与主链路 any 治理 |

> [!IMPORTANT]
> `plan-功能补齐总览.md` 已同步为 12/12 全部 `[x]`。本报告后续重点从“是否完成”转为“质量持续治理（拆分、类型、测试覆盖）”。
...
> [!NOTE]
> **Gate-R1 已于 2026-03-08 通过**（GUI 手测 + API 补证），控制台→应用→运行→License 基本链路可走通；当前 Sprint 文档状态以 `plan-产品化重构-12-sprint.md` 为准（已同步 Sprint 5 为 `[x]`）。
...
| # | 待办事项 | 来源 | 涉及文件 | 预估工作量 |
|---|---|---|---|---|
| 1 | ✅ **清理 innerHTML / v-html XSS 风险（本轮已完成）** | 代码审查 P0-1 | `AmisEditor.vue`、`amis-renderer.vue`、`MarkdownRenderer.vue` | 已完成 |
| 2 | ✅ **`any` 类型治理（Workflow/Approval 主链路 + 关键页面）** | 代码审查 P0-3 | `WorkflowDesignerPage.vue`、`WorkflowEditorPage.vue`、`WorkflowInstancesPage.vue` 等 | 本轮已收口 |
| 3 | ✅ **12 Case 前端勾选状态同步** | 文档追踪滞后 | `plan-功能补齐总览.md` | 已完成 |
| 4 | ✅ **12 Sprint 状态同步（当前已同步 Sprint 5）** | 文档追踪滞后 | `plan-产品化重构-12-sprint.md` | 部分完成 |
...
| 12 | ✅ **CI/CD 集成质量闸门**（`npm run check` 接 pipeline） | 代码审查 P2-1 | CI 配置 | 已完成 |
...
12 Sprint     ████████░░░░░░░░░░░░  ~42%（当前文档状态：1/12 已勾选）
  Gate-R1     ████████████████████  已通过
  Sprint 勾选  1/12 Sprint
```
...
1. **修复 key=idx/i 并替换为稳定业务标识**（P1 #6）
2. **推进巨型页面拆分**（P1 #5，优先 ApprovalDesigner/AppBuilder/Login）
3. **补齐关键路径单测与 E2E**（P2 #10/#11）
4. **持续推进 i18n 接入率和 token 本地存储淘汰**（P2 #13/#14）
