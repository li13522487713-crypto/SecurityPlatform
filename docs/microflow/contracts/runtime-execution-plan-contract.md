...
## 第 49 轮 Runtime FlowNavigator

后端新增 `IMicroflowFlowNavigator` / `MicroflowFlowNavigator`，唯一输入为第 48 轮生成的 `MicroflowExecutionPlan`。FlowNavigator 不读取 FlowGram JSON，不读取前端画布 document，不修改 `MicroflowAuthoringSchema`、资源、发布状态或业务数据。

导航输入字段满足度：

- `startNodeId` / `endNodeIds`：用于 root StartEvent 与 terminal EndEvent 判断。
- `nodes`：用于节点 kind、action kind、supportLevel、collectionId、parentLoopObjectId 与 trace 定位。
- `normalFlows` / `decisionFlows` / `objectTypeFlows` / `errorHandlerFlows` / `ignoredFlows`：用于按语义分桶导航，AnnotationFlow 仅在 `ignoredFlows` 中保留，不参与控制流。
- `caseValues` / `branchOrder`：用于 Boolean、Enumeration、ObjectType 的 deterministic case 选择。
- `loopCollections`：用于 LoopedActivity 骨架迭代，入口以 `startLikeNodeIds` 优先，缺失时按 collection 内无 incoming 的第一个可执行节点兜底。
- `unsupportedActions` / node `supportLevel`：用于 modeledOnly、unsupported、requiresConnector、nanoflowOnly 的导航级错误或 dry-run skip。

诊断入口：

- `POST /api/microflows/runtime/navigate`：inline AuthoringSchema 先生成 ExecutionPlan，再执行 dry-run 导航。
- `GET /api/microflows/{id}/runtime/navigate`：读取当前 schema snapshot 生成 plan 并导航。

所有入口返回 `MicroflowApiResponse<MicroflowNavigationResult>`。NavigationResult 只包含导航步骤、trace skeleton、diagnostics 与错误结构，不包含 FlowGram / WorkflowJSON。
