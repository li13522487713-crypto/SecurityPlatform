...
本矩阵用于 Runtime 契约回归。所有样例必须完成：

- 新版设计态 schema 校验
- FlowGram `workflow.nodes/workflow.edges` 投影
- 后端 runtime test-run

| sample | validate expected | runtime expected | trace expected | unsupported expected |
|--------|-------------------|------------------|----------------|----------------------|
| `sample-order-processing` | manifest 中的 errors/warnings | 后端 runtime 可生成 RunSession | 至少包含 start/action/end 中可达节点；无 FlowGram 字段 | P0 动作不进入 unsupported |
...
| `sample-large-100-nodes` | 可 validate/runtime plan | 不要求全量 test-run；如执行需受 `RUNTIME_MAX_STEPS_EXCEEDED` 保护 | 不依赖 FlowGram JSON | 大图节点不新增 Runtime unsupported |
