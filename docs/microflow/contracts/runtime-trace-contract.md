...
## 第 60 轮 Trace / Logs / Evidence

- 总控脚本 `scripts/verify-microflow-round60-full-e2e.ts` 将每个 verify 子命令输出保存到 `artifacts/microflow-e2e/round60/traces/*.log`。
- `scripts/verify-microflow-runtime-hardening.ts` 验证 TestRun 后 `GET /api/microflows/runs/{runId}` 与 `/trace` 可回读 RunSession、TraceFrame、RunLog。
- E2E 报告要求失败用例保留 `caseId`、area、expected、actual、status、severity 与 evidence 路径。
- TraceFrame / RuntimeLog 证据仍只允许运行时短 preview、变量快照和结构化 output，不保存 FlowGram JSON 或大 raw object。

## 第 61 轮保留策略

- RunSession、TraceFrame、RuntimeLog 的生产保留策略在 `Microflow:Retention` 中配置，默认 30 天、每资源最多 1000 个 run。
- Round61 不清理 `MicroflowVersion`、`MicroflowSchemaSnapshot`、`MicroflowPublishSnapshot`。
- 自动 retention service 尚未实现，内测按 `docs/microflow/release/ops-runbook.md` 执行手工 dry-run 清理。
- `MaxTraceFrames` 与 `MaxRuntimeLogs` 默认 20000，`EnableRawValueInTrace=false`。
