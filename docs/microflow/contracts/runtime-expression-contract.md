...
## 第 54 阶段 ActionExecutor 表达式集成

- `CreateVariableActionExecutor` / `ChangeVariableActionExecutor` 继续通过 `ExpressionEvaluator` 求值，并按目标 dataType 做 expectedType 检查。
- `RestCallActionExecutor` 在 testRun 中求值 url/header/query/body 表达式，但不发送真实外部请求。
- `LogMessageActionExecutor` 求值 template arguments。
- `ChangeListActionExecutor` 可求值 valueExpression；List filter/sort 的完整 item scope 和属性路径语义保留为后续深化，但当前已有明确 serverExecutable mock 行为。
- Object memberChanges 当前记录表达式文本与 preview，真实成员赋值将在 Object CRUD Actions 深化时复用同一 `ExpressionEvaluator`。
