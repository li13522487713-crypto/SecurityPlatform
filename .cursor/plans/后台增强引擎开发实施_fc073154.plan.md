...
## Phase-2：四轨并行（T02 + T03 + T04 + T06，共 100 Case）

**前置**：Phase-1 完成  
**四条轨道之间无互相依赖**，可分配给不同开发者/AI 并行推进

### Track-02：动态建模内核升级（36 Case）
...
- 审计字段模板自动注入
- 涉及文件：[src/backend/Atlas.Domain/DynamicTables/Entities/](src/backend/Atlas.Domain/DynamicTables/Entities/)
...
### Track-03：表达式与函数引擎（24 Case）

按依赖链分为 3 个子批次：

**子批次 3a - 类型与解析（T03-01 ~ T03-08）**

- `ExprType` 类型系统 + `ExprAstNode` AST 模型
- Lexer + Parser + AST JSON 序列化
- `ITypeInferencer` 基础与函数推断
- AST 编译缓存
- 涉及文件：在 [src/backend/Atlas.Core/Expressions/](src/backend/Atlas.Core/Expressions/) 或新建子模块

**子批次 3b - 函数注册与求值（T03-09 ~ T03-15）**

- `IFunctionRegistry` 字符串 / 数值 / 日期 / 转换 函数
- 集合函数 / 聚合函数 / 窗口函数
- null 传播规则

**子批次 3c - 元数据与规则（T03-16 ~ T03-24）**

- `FunctionDefinition` 实体 + CRUD + SPI
- 决策表 + 规则链执行器
- Controller + `.http`
- 函数设计页 Monaco + 可视化构造器

### Track-04：统一节点元模型（18 Case）

按依赖链分为 3 个子批次：

**子批次 4a - 元模型核心（T04-01 ~ T04-07）**
...
### Track-06：批处理引擎（22 Case）

按依赖链分为 3 个子批次：

**子批次 6a - 实体层（T06-01 ~ T06-07）**
