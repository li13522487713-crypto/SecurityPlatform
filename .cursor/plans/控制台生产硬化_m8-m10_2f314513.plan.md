...
| 里程碑 | 计划项 | 已落地 | 状态 |
| --- | --- | --- | --- |
| M8 阻塞修复 + 业务真实性 | A1/A2/A3 + B1-B5 | 全部相关源码已入库（`SetupModeMiddleware.cs` / `OrmDataMigrationService.cs` / `SetupRecoveryKeyService.cs` / `SetupConsoleService.cs` 等） | 完成 |
| M9 迁移引擎深度 | C1-C6 | C1 拓扑（`EntityTopologySorter.cs` 在位） / C2-C5 并入 `OrmDataMigrationService.cs`；**C6 SQLite→SQLite Integration Test 文件 `tests/Atlas.SecurityPlatform.Tests/Integration/OrmDataMigrationIntegrationTests.cs` 未在仓库中存在** | 部分完成 |
| M10 测试 + 安全 + 运维 UX | D1-D10 | E2E 4 spec 全部存在 / mock-switch.ts / setup-console-page.tsx 升级 / dashboard-tab.tsx 下钻 / `docs/setup-console-production-checklist.md` 与 `docs/setup-console-runbook.md` 均在 | 完成 |

**整体进度：≈ 3 / 3 里程碑（M9 集成测试用例缺位，建议补 `OrmDataMigrationIntegrationTests.cs` 后再宣告"完全闭环"）。**

里程碑全部完成后，控制台达到生产可用：
- 首装、补初始化、跨库迁移、工作空间初始化全链路真实落库
- 等保 2.0 主要红线全部覆盖（连接串加密、密码哈希、审计 IP/UA、IP 限流、首登改密、恢复密钥保护）
- xUnit ~50 单测 + ~10 integration test（**实际：integration test 计划 ~10 个，仓库中 0 个待补**）+ Playwright 4 spec 真实跑通
